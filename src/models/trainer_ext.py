...
class Trainer(object):
    ...
    def test(self, test_iter, step, cal_lead=False, cal_oracle=False):
        ...
        def _block_ngrams(c, p, n):
            ngrams_c = _get_ngrams(n, c.split())
            for s in p:
                ngrams_s = _get_ngrams(n, s.split())
                if len(ngrams_c.intersection(ngrams_s)) > 0:
                    return True
            return False
                ...
                with torch.no_grad():
                    for batch in test_iter:
                        ...
                        if (cal_lead):
                            ...
                            loss = self.loss(sent_scores, labels.float())
                            # Only calculate the place with [CLS] (not the padding ones)
                            loss = (loss * mask.float()).sum()
                            batch_stats = Statistics(float(loss.cpu().data.numpy()), len(labels))
                            stats.update(batch_stats)

                            # Eliminating the padding effect
                            # To let the true [CLS] get more chance to be selected
                        ...
                        for i, idx in enumerate(selected_ids):
                            ...
                            for j in selected_ids[i][:len(batch.src_str[i])]:
                                ...
                                # print(candidate)
                                # print(_get_ngrams(3, candidate.split()))
                                # print(_pred)
                                # exit()
                                if (self.args.block_trigram):
                                    #if (not _block_tri(candidate, _pred)):
                                    if (not _block_ngrams(candidate, _pred, self.args.ngram_blocking)):
                                        _pred.append(candidate)

                                else:
                                    _pred.append(candidate)

                                if ((not cal_oracle) and (not self.args.recall_eval) and len(_pred) == self.args.ext_sent_num):
                                    break
